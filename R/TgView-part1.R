...
GetResultPieActionMode <- function(
  object,
  limits.exprs.change = c("Xup.Yup", "Xup.Ydn", "Xdn.Yup", "Xdn.Ydn"),
  limits.action.mode = NULL, 
  color.action.mode = NULL, 
  ...
  allowed.mode <- object@pred.action$action.mode
  usage.color.mode <- object@pred.action$color.mode
  ...
  # pre-check if action mode is NULL
  if (!is.null(limits.action.mode)) {
    limits.action.mode <- allowed.mode
    color.action.mode <- usage.color.mode
  }
  # check alignment of action mode and used color 
  if (length(limits.action.mode) != length(color.action.mode)) {
    stop("Please give identical length in parameter 'limits.action.mode' and 'color.action.mode'. ")
  }
  # check action mode
  not.valid.mode <- setdiff(limits.action.mode, allowed.mode)
  if (length(not.valid.mode) > 0) {
    warning("Given invalid action mode: ", paste0(not.valid.mode, collapse = ", "), ". ")
  }
  limits.action.mode <- intersect(limits.action.mode, allowed.mode)
  if (length(limits.action.mode) == 0) {
    stop("No valid action mode.")
  }
  # aligned removing of not valid action mode
  # [TODO]
  ...
  names(this.color.pal) <- allowed.mode
  ...
  for (i.exc in limits.exprs.change) {
    ...
    if (length(tmp.mode.res) > 0) {
      ...
      gplot.sgp <- gplot.sgp + geom_bar(aes(y = cnt.percent, fill = mode), stat = "identity", position = "stack") + 
        ...
        scale_fill_manual(values = this.color.pal) + 
        coord_polar(theta = "y") + 
        scale_x_discrete(breaks = NULL) + 
        scale_y_continuous(breaks = NULL) + 
        xlab("interact pairs count") + 
        ylab(i.exc) + 
        guides(fill = guide_legend(title = legend.title)) + 
  ...
  caption.label <- paste0("X,Y: clusters that gene pairs from.\nX: ", 
    this.cluster.A, ", Y: ", this.cluster.B)
  gplot.sgp <- gplot.sgp + labs(caption = caption.label)
  #end# return
  return(list(plot = gplot.sgp, table = this.pie.df))
}
