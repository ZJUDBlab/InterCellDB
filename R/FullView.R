...
#' @param perm.pval.sides Default is 'two-side'. All allowed options are 'one-side-large' or 'one-side-small' or 'two-side'. 
...
AnalyzeInterInFullView <- function(
	...
	if (run.permutation == TRUE) {
		...
		tmp.get.perm.res.func <- function(x, perm.expression, actual.exprs, inv.gdf) {
			# actual
			...
			sum(abs(as.numeric(x["inter.Power"]) - tmp.avg.it) <= abs(nulldist.power - tmp.avg.it)) / length(nulldist.power)
		}
		if (nbrOfWorkers() == 1) {
			tmp.all.result[, "inter.Specificity"] <- as.numeric(pbapply(tmp.all.result, MARGIN = 1,
				...
				inv.gdf = tmp.involved.gdf, FUN = tmp.get.perm.res.func
			))
		} else {
