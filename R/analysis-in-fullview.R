...
AnalyzeClustersInteracts <- function(
  ...
  if ((length(sub.sel.user.merge.option) == 1) && (sub.sel.user.merge.option %in% user.merge.option.list)) {
    ...
    if (sub.sel.user.merge.option == "union" && !tmp.check.selection.X) {
      warning("Use [", sub.sel.user.merge.option, "] strategy, but user.type definition on X is not given!")
    }
    if (sub.sel.user.merge.option == "union" && !tmp.check.selection.Y) {
      warning("Use [", sub.sel.user.merge.option, "] strategy, but user.type definition on Y is not given!")
    }
  } else {
  ...
  if (!is.null(user.type.database) && 
    ...
    # for X
    if (tmp.check.selection.X == TRUE) {
      user.settings[["X.user.type"]] <- sub.sel.X.user.type
    }
    # for Y
    if (tmp.check.selection.Y == TRUE) {
