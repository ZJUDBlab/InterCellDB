...
#' @param exprs.has.added [TODO] remove This tells the program whether the expressions of genes are given in source data. If set \code{FALSE},
#' user can later add this part by [TODO-function] in this package, while set \code{TRUE}, it will be handled by this function.
#'
#'
#' @details
...
#' }
#' 
#' [TODO] remove. All other columns except 'Exprs' and above required columns are not cared. 
#' The 'Exprs' column can be given in parameter \code{DEG.table} (then parameter \code{exprs.has.added = TRUE}), or later be added
#' by [TODO-function] in this package. The data recorded under 'Exprs' column represent average expression of every gene.
#'
...
CreateInterCellObject <- function(
	...
	species,
	add.exprs = FALSE,
	exprs.data = NULL,  # could be seurat object or expression matrix
	...
	IT.InterCell.Obj <- new(
		...
		misc = list(musthave.colnames = tmp.musthave.new)
	)
...
DataPrep.AddExprs <- function(
	...
	} else {  # raw matrix or data.frame is allowed
		if (is.data.frame(exprs.data)) {
			exprs.data <- as.matrix(exprs.data)
		} else {
			if (!is.matrix(exprs.data)) {
				stop("Provided expression data should be matrix or data.frame or Seurat object (supported for >= v3).")
			}
		}
	}
	...
	if (anyDuplicated(rownames(exprs.data))) {
		warning("Duplicated genes in expression data detected. Unexpected result may be generated.")
	}
	...
	fgenes <- bind_rows(lapply(unique(fgenes$cluster), 
		...
		function(x, fgenes, exprs.data) {
			tmp.exprs <- exprs.data[, x, drop = FALSE]
			fgenes <- subset(fgenes, cluster == x)
			inds.exprs <- match(fgenes$gene, rownames(tmp.exprs))
