...
kfDB.mouse.namelist <- c("SingleCellSignalR", "NicheNet", "CellChatDB")
#kfDB.mouse.favor.color <- c("#CDD3D4", "#96CAEA", "#8CACAF")
#kfDB.mouse.favor.color <- c("#33A02C","#CAB2D6","#1F78B4")  # "#B2DF8A", "#A6CEE3"
kfDB.mouse.favor.color <- c("#74C476", "#9ECAE1", "#6BAED6")  # "#A1D99B", "#C6DBEF", 

# InterCellDB are ordered BY # experimentally validated # from curated database # by prediction # All included
kfIT.mouse.namelist <- c("InterCellDB.exp", "InterCellDB.know", "InterCellDB.pred", "InterCellDB", "InterCellDB.LR")
kfIT.mouse.favor.color <- c("#C19E77", "#E6CCA9", "#E9BC80", "#8E7152", "#D7C0BA")  # choose from ggplot2::scale_color_distiller(palette=18 <or> 8)
	# reorder IT
	kfIT.mouse.order.inds <- c(4, 1:3, 5)
	kfIT.mouse.namelist <- kfIT.mouse.namelist[kfIT.mouse.order.inds]
	#kfIT.mouse.favor.color <- kfIT.mouse.favor.color[kfIT.mouse.order.inds]
	#kfIT.mouse.favor.color <- c("#B15928", "#E31A1C", "#EF98CE", "#FDBF6F", "lightgrey")
	kfIT.mouse.favor.color <- c("#543005", "#8C510A", "#BF812D", "#DFC27D", "#FDB863")
...
#
#
kfDB.mouse.loc.all <- c("Extracellular Region", "Plasma Membrane", 
	"Cytoplasm", "Cytosol", "Cytoskeleton", 
	"Endoplasmic Reticulum", "Golgi Apparatus", 
	"Endosome", "Lysosome", "Peroxisome",
	"Mitochondrion", "Nucleus", "Other")
kfDB.mouse.type.all <- c("Actin-binding and motor protein", "Antimicrobial", "Cytokine", "Enzyme",
...
# actions
kfDB.mouse.act.mode <- c("activation", "binding", "catalysis", "expression", "inhibition", "ptmod", "reaction")
kfDB.mouse.act.effect <- c("positive", "negative", "unspecified", "undirected")

# picked GO terms
kfGO.picked.terms <- c("angiogenesis", "EMT", "complement activation", "cytokine activity")
	## GO:0001525 [angiogenesis] <BP>
	go.vessel.genes <- Tool.FindGenesFromGO("GO:0001525", genes.mouse.ref.db, go.mouse.ref.db, 
		go.use.relative = TRUE, go.relative.option = "offspring")[[1]]
	## GO:0001837 [epithelial to mesenchymal transition] <BP>
	go.emt.genes <- Tool.FindGenesFromGO("GO:0001837", genes.mouse.ref.db, go.mouse.ref.db, 
		go.use.relative = TRUE, go.relative.option = "offspring")[[1]]
	## GO:0006955 [immune response]
	#go.immune.response.genes <- Tool.FindGenesFromGO("GO:0006955", genes.mouse.ref.db, go.mouse.ref.db, 
	#	go.use.relative = TRUE, go.relative.option = "offspring")[[1]]
	## GO:0006956 [complement activation] <BP>
	go.comp.act.genes <- Tool.FindGenesFromGO("GO:0006956", genes.mouse.ref.db, go.mouse.ref.db, 
		go.use.relative = TRUE, go.relative.option = "offspring")[[1]]
	## GO:0005125 [cytokine activity] <MF>
	go.cytokine.act.genes <- Tool.FindGenesFromGO("GO:0005125", genes.mouse.ref.db, go.mouse.ref.db, 
		go.use.relative = TRUE, go.relative.option = "offspring")[[1]]
...
#
# 
tmp.mouse.genes.cnt.all <- list(scsr.mouse.genes.res, nichenet.mouse.genes.res, cellchat.mouse.genes.res, 
	intercelldb.exp.mouse.genes.res, intercelldb.know.mouse.genes.res, intercelldb.pred.mouse.genes.res, 
	intercelldb.mouse.genes.res, intercelldb.LR.mouse.genes.res)
names(tmp.mouse.genes.cnt.all) <- c(kfDB.mouse.namelist, kfIT.mouse.namelist)
...
## [fig2-A]: location based gene count, and core genes type distribution
	...
	# Location Part
	#
	check.loc.each.gene.most.conf <- tapply(anno.location.mouse.ref.db$score, anno.location.mouse.ref.db$Gene.name, max)
	...
	check.loc.egm.df <- left_join(check.loc.egm.df, anno.location.mouse.ref.db[, c("GeneID", "Gene.name", "GO.Term.target", "score")],
		by = c("gene" = "Gene.name", "loc.max.score" = "score"))
	check.loc.egm.df <- unique(check.loc.egm.df)  # 28197 rows
	# 
	tmp.use.loc.category <- kfDB.mouse.loc.all  # change to use for different purpose
	tmp.use.loc.replace <- FALSE
	tmp.use.DB.namelist <- c(kfDB.mouse.namelist, kfIT.mouse.namelist)
	#
	tmp.check.loc.alldb <- lapply(tmp.mouse.genes.cnt.all[tmp.use.DB.namelist], 
...
#
	...
	tmp.use.DB.namelist.for.unip <- c(kfDB.mouse.namelist, kfIT.mouse.namelist)
	tmp.DBgenes.unip.countlist <- lapply(seq_along(tmp.mouse.genes.cnt.all[tmp.use.DB.namelist.for.unip]),
		inside.genes.cnt.all = tmp.mouse.genes.cnt.all[tmp.use.DB.namelist.for.unip], unip.list = tmp.use.mf.unip.genelist,
	...
	tmp.unip.order.ref <- abs(as.numeric(unlist(lapply(tmp.use.mf.unip.sel, 
		...
		function(x, ref.df) {
			...
			# implicitly 5 vs 3
			mean(tmp.use.it.df[4:8, "log.unip.cnt"]) - mean(tmp.use.it.df[1:3, "log.unip.cnt"])
		}))))
	...
	# plot settings
	tmp.for.unip.colour <- c(kfDB.mouse.favor.color, kfIT.mouse.favor.color)
	names(tmp.for.unip.colour) <- c(kfDB.mouse.namelist, kfIT.mouse.namelist)
	# plot # total 15 Keywords, split to 3 group by hand
	...
	# plot
	#tmp.unip.plot.use.group <- tmp.unip.plot.group.3  # [NOTE] select one group once
...
# pasted gene pairs
cmp.pairs.db.list <- list(remap.scsr.pairs.mouse, 
	remap.nichenet.pairs.mouse, remap.cellchat.pairs.mouse,
	ref.intercelldb.pairs.mouse.experiments, ref.intercelldb.pairs.mouse.knowledge, 
	ref.intercelldb.pairs.mouse.prediction, ref.intercelldb.pairs.mouse, 
	ref.intercelldb.pairs.mouse.LR)
cmp.pairs.db.list[4:8] <- cmp.pairs.db.list[4:8][kfIT.mouse.order.inds]
names(cmp.pairs.db.list) <- c(kfDB.mouse.namelist, kfIT.mouse.namelist)

# position-aligned gene pairs
pos.pairs.db.list <- list(pos.scsr.pairs.mouse, 
	pos.nichenet.pairs.mouse, pos.cellchat.pairs.mouse,
	pos.intercelldb.pairs.mouse.experiments, pos.intercelldb.pairs.mouse.knowledge,
	pos.intercelldb.pairs.mouse.prediction, pos.intercelldb.pairs.mouse,
	pos.intercelldb.pairs.mouse.LR)
pos.pairs.db.list[4:8] <- pos.pairs.db.list[4:8][kfIT.mouse.order.inds]
names(pos.pairs.db.list) <- c(kfDB.mouse.namelist, kfIT.mouse.namelist)
...
# fetch all intersect-ed ligand and receptor genes (genes that exist in all database and identified as ligand or receptor)
	# receptor
	tmp.receptor.cnt.all <- list(sel.mouse.receptor.scsr.res, 
		sel.mouse.receptor.nichenet.res, sel.mouse.receptor.cellchat.res, sel.mouse.receptor.intercelldb.res)
	names(tmp.receptor.cnt.all) <- c(kfDB.mouse.namelist, "InterCellDB")
	tmp.allagree.receptor.genes <- Reduce(intersect, tmp.receptor.cnt.all)
	# ligand
	tmp.ligand.cnt.all <- list(sel.mouse.ligand.scsr.res, 
		sel.mouse.ligand.nichenet.res, sel.mouse.ligand.cellchat.res, sel.mouse.ligand.intercelldb.res)
	names(tmp.ligand.cnt.all) <- c(kfDB.mouse.namelist, "InterCellDB")
	...
	tmp.ribbon.gp.matchcnt.df <- bind_rows(lapply(tmp.for.gp.allagree.genes, 
	...
	tmp.ribbon.order.ref.df <- tmp.ribbon.gp.matchcnt.df[which(tmp.ribbon.gp.matchcnt.df$DBname == "InterCellDB"), ]
	...
	#
	tmp.ribbon.gp.matchcnt.df <- left_join(tmp.ribbon.gp.matchcnt.df, tmp.ribbon.gene.order.df, by = c("Gene" = "Gene"))
	...
	tmp.ribbon.db.colour <- c(kfDB.mouse.favor.color, kfIT.mouse.favor.color)
	names(tmp.ribbon.db.colour) <- c(kfDB.mouse.namelist, kfIT.mouse.namelist)
	# top genes
...
# fetch ref and cmp database list
	tmp.overlap.ref.gp.db.list <- cmp.pairs.db.list[kfDB.mouse.namelist]
	tmp.overlap.tocmp.gp.db.list <- cmp.pairs.db.list[kfIT.mouse.namelist]
	...
	# plot data prep
	tmp.gpairs.corel.df$DBx <- factor(tmp.gpairs.corel.df$DBx, levels = names(tmp.overlap.tocmp.gp.db.list))
	tmp.gpairs.corel.df$DBy <- factor(tmp.gpairs.corel.df$DBy, levels = names(tmp.overlap.ref.gp.db.list))
...
# subset the GOlist
tmp.chk.genes.picked.GOlist <- tmp.sel.GOterms.big
#
# get genes
...
# plot data prep
tmp.picked.go.res.df$DBname <- factor(tmp.picked.go.res.df$DBname, levels = c(kfDB.mouse.namelist, kfIT.mouse.namelist))
tmp.picked.go.res.df$GOterm <- factor(tmp.picked.go.res.df$GOterm, levels = names(tmp.picked.go.rel.genelist))

# only used in sml (NOT USED anymore)
#
tmp.picked.go.res.df$log.pairs.cnt <- sapply(tmp.picked.go.res.df$log.pairs.cnt, function(x) {if (x < 0) x <- 0; x})
#
